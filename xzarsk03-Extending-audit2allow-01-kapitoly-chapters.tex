...
\begin{description}
    ...
    \item [\texttt{type}] Type defaults to the type of the parent directory
        ...
        % TODO: make it clear, low or current?
...
on extended permission AV rules generation. Tests are implemented in the
\texttt{test\_audit2allow.py} module.

%\extrarowsep=5pt
\tabulinesep=10pt
\begin{longtabu}{|l|X|} \hline
    \texttt{Audit2allowTests.test\_xperms()} & Test that \texttt{audit2allow -x}
...
convert the fields to extended permissions. Tests are implemented in the
\texttt{test\_audit.py} module.

\subsubsection{Testing \texttt{AVCMessage.\_\_init\_\_()}}
\begin{description}
    \item [\texttt{TestAVCMessage.test\_defs()}] Test that
        \texttt{AVCMessage.ioctlcmd} is \texttt{None}.
\end{description}

\subsubsection{Testing \texttt{AVCMessage.from\_split\_string()}}
\begin{description}
    \item [\texttt{TestAVCMessage.test\_xperms()}] Test that the
        \texttt{ioctlcmd} field is parsed.
    \item [\texttt{TestAVCMessage.test\_xperms\_invalid()}] Test message with
        invalid value in the \texttt{ioctlcmd} field.
    \item [\texttt{TestAVCMessage.test\_xperms\_without()}] Test message without
        the \texttt{ioctlcmd} field.
\end{description}

\subsubsection{Testing \texttt{AVCMessage.to\_access()}}
\begin{description}
    \item [\texttt{TestAuditParser.test\_parse\_xperms()}] Test that correct
        access vectors are generated from a set of AVC denial messages.
\end{description}
...
\begin{description}
    \item [\texttt{TestAccessVector.test\_init()}] Test that
...
Test cases:
\begin{description}
    \item [\texttt{TestAccessVector.test\_merge\_noxperm()}] Test merging two
        AVs without extended permissions.
    \item [\texttt{TestAccessVector.test\_merge\_xperm1()}] Test merging AV that
        contains extended permissions with AV that does not.
    \item [\texttt{TestAccessVector.test\_merge\_xperm2()}] Test merging AV that
        does not contain extended permissions with AV that does.
    \item [\texttt{TestAccessVector.test\_merge\_xperm\_diff\_op()}] Test
        merging two AVs both containing extended permissions, but with different
        operations.
    \item [\texttt{TestAccessVector.test\_merge\_xperm\_same\_op()}] Test
        merging two AVs both containing extended permissions with the same
        operation.
\end{description}
...
Test cases:
\begin{description}
    \item [\texttt{TestAccessVectorSet.test\_add\_av\_first()}] Test adding
        first access vector to the access vector set.
    \item [\texttt{TestAccessVectorSet.test\_add\_av\_second()}] Test adding
        second AV to the set with same source and target context and class.
    \item [\texttt{TestAccessVectorSet.test\_add\_av\_with\_msg()}] Test adding
        audit message.
\end{description}
...
Test cases:
\begin{description}
    \item [\texttt{TestAccessVectorSet.test\_add()}] Test adding access vector
        to the set.
\end{description}
...
\subsubsection{Testing \texttt{PolicyGenerator.\_\_init\_\_()}}
\begin{description}
    \item [\texttt{TestPolicyGenerator.test\_init()}] TODO
\end{description}

\subsubsection{Testing \texttt{PolicyGenerator.set\_gen\_xperms()}}
\begin{description}
    \item [\texttt{TestPolicyGenerator.test\_set\_gen\_xperms()}] TODO
\end{description}

\subsubsection{Testing \texttt{PolicyGenerator.add\_access()}}
\begin{description}
    \item [\texttt{TestPolicyGenerator.test\_av\_rules()}] TODO
    \item [\texttt{TestPolicyGenerator.test\_ext\_av\_rules()}] TODO
\end{description}
...
\subsubsection{Testing \texttt{XpermSet} class}
\begin{description}
    \item [\texttt{TestXpermSet.test\_init()}] TODO
    \item [\texttt{TestXpermSet.test\_normalize\_ranges()}] TODO
    \item [\texttt{TestXpermSet.test\_add()}] TODO
    \item [\texttt{TestXpermSet.test\_extend()}] TODO
\end{description}

\subsubsection{Testing \texttt{AVExtRule} class}
\begin{description}
    \item [\texttt{TestAVExtRule.test\_init()}] TODO
    \item [\texttt{TestAVExtRule.test\_rule\_type\_str()}] TODO
    \item [\texttt{TestAVExtRule.test\_from\_av()}] TODO
    \item [\texttt{TestAVExtRule.test\_from\_av\_self()}] TODO
    \item [\texttt{TestAVExtRule.test\_to\_string()}] TODO
\end{description}
...
% ------------------------------------------------------------------------------
\section{Testing Implementation of Mislabeled Files}
% STATUS: 2
% TODO: write more stuff
