...
\label{avrules}
The AV rules supports type enforcement within SELinux. They control what access
do processes get. The \texttt{allow} rule grants process an access to an object.
Syntax:
\begin{lstlisting}[language=te]
...
There are three other AV rules that follow the same syntax as the \texttt{allow}
rule:
\begin{description}
