...
only ioctl whitelisting, but they provide generic tool that can be used in
...
\begin{description}
    ...
    \item [\texttt{source\_type}, \texttt{target\_type}, \texttt{class}] are
        source type, target type, and object class, same as with normal AV rule.
...
are allowed. With \texttt{allowxperm} rule, only specified values are allowed.
