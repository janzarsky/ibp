...
policy to allow the operations that were previously denied before.
...
that should not be solved by adding new rules to the policy. As a~result, users
end up with policy rules that give processes too much permissions, making whole
...
The users, not familiar with SELinux, cannot recognize limitations of the
...
improvements to audit2allow. The fifth one describes unit and integration tests
of implemented improvements to audit2allow.
...
methods such as file permissions. Users can grant insecure file permissions to
others or gain access to the files that they do not need \cite{selinuxguide}:
\begin{itemize}
...
SELinux. When an action is denied, it is reported via an ``access denied'' error
...
\begin{description}
    ...
    \item [SELinux and proc filesystem] Userspace tools communicate with the
        kernel security server via the \texttt{/proc} and
        \texttt{/sys/fs/selinux} filesystem.
...
\begin{description}
    ...
    \item [Object] is a~system resource such as a file, a socket, a pipe, TCP or
        UDP port, a network interface, a semaphore or shared memory segment.
\end{description}
...
Linux Security Module hooks call security checks in SELinux.
...
mechanisms. RBAC as general concept is based on users, roles, and permissions,
...
the \texttt{system\_u} user is associated to roles \texttt{unconfined\_r} and
...
classification. Process running at a certain security level can:
\begin{itemize}
...
transition \cite[pp.~43--47]{tsn}. For example, \texttt{systemd} process running
as \texttt{init\_t} needs to start the Apache HTTP Server as \texttt{httpd\_t}.
...
which access processes get. The audit2allow utility generates access vector
...
permission sets, extended permission AV rules add arbitrary number of 256~bit
increments. The extended permission AV rules are currently (as of policy version
31) used only for ioctl whitelisting, but they provide generic tool that can be
used for more granular control over an operation in the future \cite{selinuxmailxperms}.
...
The audit2allow utility can output the policy rules directly to stdout or a
...
After parsing of AVC message, the denial is analyzed in the \texttt{audit2why.c}
...
The proper way how to allow the process to bind on port number 43 would be to
