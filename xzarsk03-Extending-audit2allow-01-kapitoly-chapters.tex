...
analyzes SELinux denials and generates policy rules that can be loaded into the
security policy to allow the operations that were previously denied.
...
do not require new rules in policy. As a result, users end up with policy rules
that give processes too much permissions, making whole system more vulnerable.

In other situations, the audit2allow utility provides nonfunctional solutions
because it is not aware of recently added features of SELinux. There are policy
statements that provide more granular control over given permissions. The
audit2allow utility is unable to detect that the denial was caused by these
statement and fails to provide working solution. SELinux policy developers
...
allow the operations that were denied.

In case of mislabeled TCP or UDP ports, audit2allow can detect that the port
needs application-specific label. Creating new labels for ports is a task that
requires certain knowledge of SELinux a the solution is not straightforward
Suggesting labels for ports can be imlemented in the future.
