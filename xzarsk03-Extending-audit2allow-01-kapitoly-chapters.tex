...
SELinux can enforce access to system ports. For example, in Fedora 27, there are
several hundred \texttt{portcon} rules that label TCP and UDP ports. Example:
\begin{lstlisting}
$ seinfo --portcon
...
When a process tries to access port and it is denied by policy, AVC message is
...
this port with a context specific to the application. The audit2allow suggest
