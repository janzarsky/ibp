...
SELinux userspace several tools and libraries \cite[pp.~19--22]{tsn}:
\begin{description}
...
\cite[p.~24]{tsn}. Every Linux user is associated to a SELinux user. For
...
to one or more roles \cite[p.~24]{tsn}. Each role can access only domain types
...
\label{te}
SELinux uses a type enforcement for enforcing mandatory access control
...
\cite[pp.~48--53]{tsn}. Security context is extended by level or range entry.
...
assigned to every subject and object \cite[pp.~27--28]{tsn}. The security
context is sometimes reffered to as \emph{security label} or just \emph{label}.
The security context is a string in the following form:
\begin{lstlisting}
...
policy statements \cite[pp.~31--33]{tsn}.
...
\cite[pp.~43--47]{tsn}. For example, \texttt{systemd} running as \texttt{init\_t}
needs to start the Apache HTTP Server as \texttt{httpd\_t}. Apache executables
are labeled \texttt{httpd\_exec\_t}. The following policy rule allows the
transition:
\begin{lstlisting}
...
is required \cite[pp.~47--48]{tsn}. For example when an X server creates a file
...
SELinux policy. This section describes mainly SELinux policy statements that are
...
Multi-Level Security, SELinux assigns subjects and objects user, role, and type
identifiers. SELinux users are defined using the \texttt{user} statement. Users
are assigned one or more roles. SELinux roles are defined using the
\texttt{role} statement. Roles are assigned types that they can access. SELinux
types are declared using the \texttt{type} statement.
...
\texttt{roleattribute} statements. Types can also be grouped together using the
\texttt{attribute} and \texttt{typeattribute} statements. Type aliases can be
defined using \texttt{typealias} statements.
...
\end{lstlisting}
When loading this module, some other module or base policy must define types
\texttt{nscd\_t} and \texttt{nscd\_var\_run\_t}, and class \texttt{nscd} with
...
The \texttt{chcon} command changes the SELinux context of files
\cite{selinuxguide}. User must have the permissions to relabel files. The
changes made by \texttt{chcon} are overwritten by a file system relabel or
running of \texttt{restorecon}.
...
The \texttt{restorecon} utility uses file contexts files to get default security
contexts of files (see section \ref{filecontexts}). For example, when file
\texttt{/var/www/html/my\_file.html} is mislabeled, the denial should be fixed
by running \texttt{restorecon} on the file:
\begin{lstlisting}
...
mislabeled file and suggest solution using the \texttt{restorecon} utility.
