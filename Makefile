...
CO=xzarsk03-Extending-audit2allow
...
normostrany:
	echo "scale=2; (`cat $(CO)-[01]*.tex | tr '$$' 'X' | grep -v '\input' | detex | wc -c` + `grep -Rh '% COUNT:' | grep -v .swp | cut -d' ' -f 3 | awk '{ total += $$1 } END { print total }'`)/1800;" | bc
